...
namespace App.Scripts.Scenes.SceneHeroes.Bootstrap.Installers
{
    ...
    {
        [SerializeField]
        private ViewSwitchNavigator viewSwitchNavigator;
        ...
        [SerializeField]
        private ConfigViewObstacle configViewSpriteCell;

        [SerializeField]
        private ConfigUnitViews configUnits;

        [SerializeField]
        private ViewPathGrid.Config pathConfig;

        [SerializeField]
        private PathLineDrawerMono lineDrawer;
        ...
        protected override void OnInstallBindings()
        {
            ...
            FactoryViewCell factoryViewCell = new(prefabView);

            Container.SetService<IFactory<ViewCell>, FactoryViewCell>(factoryViewCell);
            Container.SetService<IFactory<int, ViewCell>, FactoryViewObstacle>(
                new FactoryViewObstacle(configViewSpriteCell, new FactoryViewCell(prefabView))
            );
            Container.SetService<IFactory<UnitType, ViewCell>, FactoryViewUnit>(
                new FactoryViewUnit(factoryViewCell, configUnits)
            );

            Container.SetService<IObstacleConfiguration, ConfigViewObstacle>(configViewSpriteCell);

            Container.SetService<IViewPath, ViewPathGrid>(
                Container.CreateInstanceWithArguments<ViewPathGrid>(pathConfig, lineDrawer)
            );
        }
    }
}
