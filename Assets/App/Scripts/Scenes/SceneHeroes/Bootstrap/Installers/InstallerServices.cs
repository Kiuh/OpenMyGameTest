...
namespace App.Scripts.Scenes.SceneHeroes.Bootstrap.Installers
{
    public class InstallerServices : MonoInstaller
    {
        ...
        [SerializeField]
        private ServiceLevelProviderJson.Config config;
        ...
        {
            Container.SetService<IServiceLevelProvider, ServiceLevelProviderJson>(
                new ServiceLevelProviderJson(config)
            );
            ...
            Container.SetService<IObstacleMap, ObstacleMapGrid>(
                Container.CreateInstance<ObstacleMapGrid>()
            );
            Container.SetService<IFactory<IUnitInfo, Unit>, FactoryUnit>(
                Container.CreateInstance<FactoryUnit>()
            );

            Container.SetService<IServiceUnitNavigator, ServiceUnitNavigator>(
                new ServiceUnitNavigator()
            );
        ...
        {
            HandlerInputCellClickContainer handlerClickContainer = new();

            handlerClickContainer.AddHandler(InputHandlersKeys.HeroMove, new HandlerMoveHero());
            handlerClickContainer.AddHandler(
                InputHandlersKeys.ChangeObstacle,
                Container.CreateInstance<HandlerUpdateObstacle>()
            );
            handlerClickContainer.AddHandler(
                InputHandlersKeys.FindPath,
                Container.CreateInstance<HandlerFindPath>()
            );

            Container.SetService<IHandlerFieldClick, HandlerInputCellClickContainer>(
                handlerClickContainer
            );
            Container.SetService<IHandlerInputSelector, HandlerInputCellClickContainer>(
                handlerClickContainer
            );
        }
    }
