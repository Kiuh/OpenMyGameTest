...
namespace App.Scripts.Scenes.SceneHeroes.Features.GameInput.Services.Handlers
{
    public class HandlerUpdateObstacle : IHandlerFieldClick
    {
        ...
        public HandlerUpdateObstacle(
            IObstacleMap obstacleMap,
            IObstacleConfiguration obstacleConfiguration
        )
        {
        ...
        {
            int obstacle = _obstacleMap.GetAt(cellIndex);
            int index = _availableObstacles.IndexOf(obstacle);
            index++;
